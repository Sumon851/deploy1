...
try:
    ...
    if not landuse == '':
        if landuse == 'block':
            ...
            f = float(factor[index])
    st.write(':blue[Soil information]')


    # expander=st.expander('i')
    # help_soil=('Soil type is one of the most important parameters for any kind of investigation, design, and construction. Client must input the soil types and their extents in the platform to make the preliminary evaluation of the area possible.')
    def soil_q():
        ...
        for idx, types in enumerate(soils):
            ...
            if idx < 3:
                number = col[idx].number_input(f'{types} %', max_value=max_value, step=1)
            else:
                number = col2[idx - 3].number_input(f'{types} %', step=1)
